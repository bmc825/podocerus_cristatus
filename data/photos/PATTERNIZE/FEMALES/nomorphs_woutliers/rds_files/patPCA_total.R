...
#'
#' @param patternize_list List of raster objects, where each element is an individual, and
...
#'
#' @export
patPCA_total <- function(patternize_list, which_colors = "all",
                         PCx = 1, PCy = 2,
                         plotting = TRUE,
                         quietly = TRUE) {
  ...
  if (sum(which_colors != "all") > 0) {
    ...
    patternize_list <- patternize_list[which_colors]
  ...
  summedRasters <- lapply(patternize_list,
